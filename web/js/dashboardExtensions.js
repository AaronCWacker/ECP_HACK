...
class AxiosHelper {
    constructor(targetData, successFn) {
        this.baseData = targetData || 'defaultData';
        this.successFn = successFn;
        this.resultData = document.getElementById(this.baseData);
        this.resultData.innerHTML = 'Processing ...';
    }
    
    responseThen(response) {
        let my_results = '<h4>Result</h4>' +
            '<h5>Status:</h5> ' +
    ...
    responseCatch(error) {
        let my_results = '<h3>' + error + '<h3><h4>Result</h4><h5>Message:</h5> ' + error.message;
        this.resultData.innerHTML = 'Failed. ';
        if (error.response) {
            my_results += '<h5>Status:</h5> ' +
            ...
            my_results += '<h5>No error response.</h5> ';
